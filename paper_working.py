...
N_JOBS        = max(1, multiprocessing.cpu_count() - 1)  # Use all available cores minus one
USE_SAVED_FEATURES = True  # Set to True to use precomputed features
SAVE_DIR      = 'features'
...
def compute_features_for_all(raws, labels, save_dir=SAVE_DIR):
    """Compute and save features for all trials."""
    ...
    print(f"   → Saved features to {save_dir}/X.npy and y.npy", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        if USE_SAVED_FEATURES and os.path.exists(os.path.join(SAVE_DIR, 'X.npy')):
            print(f"Loading precomputed features from {SAVE_DIR}...", flush=True)
            X = np.load(os.path.join(SAVE_DIR, 'X.npy'))
            y = np.load(os.path.join(SAVE_DIR, 'y.npy'))
            print(f"   → Loaded {X.shape[0]} samples", flush=True)
        ...
        # Split data
        print("4) Splitting data...", flush=True)
